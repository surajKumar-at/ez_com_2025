...
import { 
  createSapOrder, 
  formatSapPayload 
} from '../services/sapService.ts';

export async function handleSyncOrder(req: Request): Promise<Response> {
  ...
  console.log('Syncing order with SAP:', orderId);
  ...
  // Format the payload and create the order in SAP
  ...
  const payload = formatSapPayload(order, soldToId, sapCreds);
  
  try {
    const { sapOrderNumber, error } = await createSapOrder(payload, sapCreds);
    ...
    if (sapOrderNumber) {
      ...
      if (updateError) {
        console.error('Error updating order with SAP order number:', JSON.stringify(updateError, null, 2));
        return new Response(
          JSON.stringify({
            error: 'SAP order created but failed to update local order',
            sapOrderNumber,
            details: updateError
          }),
          {
      ...
      console.log('Order updated with SAP order number');

      // Send confirmation email
      try {
        console.log('Sending SAP order confirmation email...');
        ...
        if (userError || !userData.user?.email) {
          console.warn('Could not fetch user email for order confirmation:', userError);
        } else {
          const { error: emailError } = await supabase.functions.invoke('send-sap-order-confirmation', {
            body: {
              ...
              apiSource: sapCreds.sap_system_type || 'S4'
            }
          });
          ...
          if (emailError) {
            ...
            console.log('SAP order confirmation email sent successfully');
          }
        }
      } catch (emailError) {
    ...
    console.error('Error during SAP order creation:', error);
    return new Response(
      JSON.stringify({ 
        error: 'Error creating SAP order', 
