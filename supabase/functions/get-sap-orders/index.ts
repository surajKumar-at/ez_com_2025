...
serve(async (req: Request) => {
  ...
  console.log("Edge function get-sap-orders called");
  ...
  try {
    ...
    try {
      ...
      const apiPath = credentialsData.api_path;

      if (!sapUsername || !sapPassword || !server || !apiPath) {
      ...
      console.log("Using SAP API Base URL:", sapApiBaseUrl);
      ...
      // Add orderby to sort by SalesOrder in descending order
      url += `&$orderby=SalesOrder desc`;
      
      console.log("Calling SAP API URL with token and pagination");
      ...
      console.log(`SAP orders API call took ${(sapCallEnd - sapCallStart).toFixed(2)}ms`);
